...
bool do_fuzzy_experiment = false;
...
void parse_input(int argc, char *argv[]){
    for(int i =1; i < argc; i++){
        ...
        else if(strcasecmp(argv[i], "-expf") == 0){
            do_fuzzy_experiment = true;
        }
    }
}

void run_tests(sparsegraph sg, char*input_file){
    //graphutil_test(sg);
    //cache_test(sg);
    //equivalence_test(sg);
    //twin_test(sg);
...
int main(int argc, char *argv[]){
    ...
    if(cascade){
        ...
        // if(statistics > 1)c.print_info(); // TODO: implement
